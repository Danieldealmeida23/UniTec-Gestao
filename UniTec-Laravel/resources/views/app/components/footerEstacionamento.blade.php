...
@php
        ...
        $consulta_financeira = $financeiro_estacionamento
                                  ->where('id_usuario', $_SESSION['pessoa'])
                                  ->where('status_pg', '')
                                  ->orWhere('status_pg', null)->get();

        $consulta_id_estacionamento = (new FinanceiroEstacionamento)
                                          ->where('id_usuario', $_SESSION['pessoa'])
                                          ->where('status_pagamento', '')->get();

        echo "<h6 class='text-uppercase font-weight-bold'><strong>Liberação do Veículo</strong></h6>";
        if(isset($consulta_financeira[0]) 
        && isset($consulta_id_estacionamento[0]))
        {
          if(($consulta_financeira[0]->status_pg == "" 
          || $consulta_financeira[0]->status_pg == null) 
          && $consulta_financeira[0]->id_uso_estacionamento == $consulta_id_estacionamento[0]->id_uso_estacionamento)
          {
